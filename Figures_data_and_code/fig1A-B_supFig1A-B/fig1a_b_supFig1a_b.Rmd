...
```{r}
clinical_data_patient <- read.delim("fig1b_supFig1b_data/data_clinical_patient.txt" , comment.char = "#")
clinical_data_sample <- read.delim("fig1b_supFig1b_data/data_clinical_sample.txt" , comment.char = "#")

clinical_patients_samples_ids_relation <- clinical_data_sample[,c("PATIENT_ID","SAMPLE_ID")]

mutational_data <- data.table::fread("fig1b_supFig1b_data/data_mutations_extended.txt")
```
...
write.table(mutational_data_breast_primary, 
            "fig1b_supFig1b_data/Primary (318\\8526, 4.1%) .txt", 
            col.names = T, 
            row.names = F, 
            quote = F,
            sep = "\t",
...
write.table(mutational_data_breast_metastatic, 
            "fig1b_supFig1b_data/Metastatic (386\\6731, 6.3%) .txt", 
            col.names = T, 
            row.names = F, 
            quote = F,
            sep = "\t",
...
```{r}
CNA <- data.table::fread("fig1b_supFig1b_data/data_CNA.txt")
...
write.table(mutational_data_breast_primary_ERBB2_amp, 
            "fig1b_supFig1b_data/Primary (40\\929, 4.3%) (ERBB2).txt", 
            col.names = T, 
            row.names = F, 
            quote = F,
            sep = "\t",
...
write.table(mutational_data_breast_metastatic_ERBB2_amp, 
            "fig1b_supFig1b_data/Metastatic (46\\676, 6.8%) (ERBB2).txt", 
            col.names = T, 
            row.names = F, 
            quote = F,
            sep = "\t",
